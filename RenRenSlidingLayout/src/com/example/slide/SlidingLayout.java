...
public class SlidingLayout extends LinearLayout implements OnTouchListener {
	...
	/**
	 * 绑定集合,绑定监听侧滑事件的View，即在绑定的View进行滑动才可以显示和隐藏左侧布局。
	 * @author jiangqingqing
	 * @time 2013/11/05  18:00
	 */
	public void setScrollEvent()
	{
		if(null!=mViewLists&&mViewLists.size()>=0)
		{
			int size=mViewLists.size();
			Log.i(TAG, "侧滑:"+size);
			for(int i=0;i<size;i++)
			{
			   mViewLists.get(i).setOnTouchListener(this);	
			}
		}
	...
	/**
	 * 添加需要侧滑事件的View到集合中
	 * @param pView 需要绑定的View对象
	 * @author jiangqingqing
	 * @time 2013/11/05  18:00
	 */
	public void addSrollEvent(View pView)
	{
		if(mViewLists==null)
		{
			mViewLists=new ArrayList<View>();
		}
		mViewLists.add(pView);
	}
